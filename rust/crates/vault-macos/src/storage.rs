...
use std::io::Write;
#[cfg(target_os = "macos")]
use vault_core::PlatformError;
use vault_core::{EncryptedStorage, MountedVolume, PlatformResult};
...
impl EncryptedStorage for MacosEncryptedStorage {
    fn format_new(&self, _key: &[u8]) -> PlatformResult<()> {
        #[cfg(target_os = "macos")]
        {
            ...
            self.run_hdiutil_with_key(&args, _key)?;
            Ok(())
        }
    ...
    fn open(&self, _key: &[u8]) -> PlatformResult<MountedVolume> {
        #[cfg(target_os = "macos")]
        {
